    ...
    return (
      <div style={{ width: '100%' }}>
        ...
        <ResponsiveContainer width="100%" height={300}>
          <LineChart width={150} height={80} data={data} onMouseMove={onMouseMove}
      onMouseOut={onMouseOut}  >
        ...
        <linearGradient id="colorUv" x1="0%" y1="0" x2="100%" y2="0">
          ...
          <Line type="monotone" dataKey={props.yaxis} stroke="url(#colorUv)" dot={false} activeDot={{ onClick:handleClick }} />
          </LineChart>
        </ResponsiveContainer>
  ...
  if (active && payload && payload.length) {
    return (
      <div className="custom-tooltip" style={{backgroundColor:"#fff", outline:"2px solid #ADD8E6", padding:"10px"}}>
        ...
        {payload[0].payload.Event && <p className="label">{payload[0].payload.Event}</p>}
      </div>
    );
  }
